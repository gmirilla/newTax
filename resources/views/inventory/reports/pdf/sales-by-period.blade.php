...
<h1>Sales by Period Report</h1>
<div class="meta">
    Period: {{ $filters['from'] }} to {{ $filters['to'] }} &bull;
    Grouped by: {{ ucfirst($filters['group_by']) }} &bull;
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="r">{{ number_format($row->cogs, 2) }}</td>
            <td class="r {{ $row->gross_profit >= 0 ? 'profit' : 'loss' }}">{{ number_format($row->gross_profit, 2) }}</td>
        </tr>
        @empty
        <tr><td colspan="5" style="text-align:center;padding:10px;color:#6b7280;">No confirmed sales in this period.</td></tr>
        @endforelse
    </tbody>
    @if($rows->count())
    <tfoot>
        <tr>
            <td>TOTALS</td>
            <td class="r">{{ number_format($totals['orders']) }}</td>
