                        ...
                        <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-3 text-sm">
                                <div class="col-span-2">
                                    <label class="block text-xs font-medium text-gray-700">Company / Customer Name *</label>
                                ...
                                <div>
                                    ...
                                    <input type="text" x-model="newCustomer.phone" placeholder="+234..."
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700">TIN</label>
                                    <input type="text" x-model="newCustomer.tin" placeholder="1234567-0001"
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700">RC Number</label>
                                    <input type="text" x-model="newCustomer.rc_number" placeholder="RC123456"
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm">
                                </div>
                                <div class="col-span-2">
                                ...
                                <div class="col-span-2 flex items-center gap-2">
                                    <input type="checkbox" x-model="newCustomer.is_company" id="nc_is_company"
                                           class="rounded border-gray-300 text-green-600">
                                    <label for="nc_is_company" class="text-xs text-gray-600">This is a company (not an individual)</label>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-end gap-3 pt-2">
                                ...
                                <button type="button" @click="saveNewCustomer()"
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="index">
                            <tr class="border-b">
                                ...
                                <td class="py-2 pr-2 text-right text-sm" x-text="formatNaira(item.subtotal)"></td>
                                <td class="py-2 pr-2 text-right text-sm text-blue-600" x-text="formatNaira(item.vatAmount)"></td>
                                <td class="py-2 pr-2 text-right text-sm font-medium" x-text="formatNaira(item.total)"></td>
                                <td class="py-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <textarea name="terms" rows="2" placeholder="Payment terms..."
                          class="mt-2 w-full border-gray-300 rounded-md text-sm focus:ring-green-500 focus:border-green-500">{{ old('terms', 'Payment due within 30 days. Please quote invoice number on remittance.') }}</textarea>
            </div>

            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-700 mb-3">Invoice Summary</h3>
                <dl class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        <dt class="text-gray-500">Subtotal</dt>
                        <dd x-text="'₦' + formatNumber(totals.subtotal)"></dd>
                    </div>
                    <div class="flex justify-between text-blue-700" x-show="vatApplicable">
                        <dt>VAT (7.5%)</dt>
                        <dd x-text="'₦' + formatNumber(totals.vatAmount)"></dd>
                    </div>
                    <div class="flex justify-between text-red-700" x-show="whtApplicable">
                        <dt x-text="'WHT (' + whtRate + '%) — deducted by customer'"></dt>
                        <dd x-text="'- ₦' + formatNumber(totals.whtAmount)"></dd>
                    </div>
                    <div class="flex justify-between border-t pt-2 font-bold text-lg">
                        <dt>Total Due</dt>
                        <dd class="text-green-700" x-text="'₦' + formatNumber(totals.grandTotal)"></dd>
                    </div>
                </dl>
...
function invoiceForm() {
    return {
        ...
        newCustomerError: '',
        newCustomer: { name: '', email: '', phone: '', tin: '', rc_number: '', address: '', is_company: true },

        async saveNewCustomer() {
            if (!this.newCustomer.name.trim()) {
                this.newCustomerError = 'Customer name is required.';
                return;
            }
            ...
            try {
                ...
                const data = await res.json();
                if (!res.ok) {
                    this.newCustomerError = data.message || 'Failed to save customer.';
                    return;
                }
                // Add to select and pre-select it
                ...
                this.newCustomer = { name: '', email: '', phone: '', tin: '', rc_number: '', address: '', is_company: true };
            } catch (e) {
                this.newCustomerError = 'Network error. Please try again.';
            } finally {
        ...
        addItem() {
            this.items.push({ description: '', quantity: 1, unitPrice: 0, subtotal: 0, vatAmount: 0, total: 0 });
        },
        ...
        calculateTotals() {
            ...
            this.totals.grandTotal = this.totals.subtotal + this.totals.vatAmount - this.totals.whtAmount;
        },

        formatNaira(val) { return '₦' + this.formatNumber(val); },
        formatNumber(val) { return Number(val).toLocaleString('en-NG', { minimumFractionDigits: 2 }); }
    };
}
</script>
@endpush
