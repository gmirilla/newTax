{{--
    ...
    Variables expected from parent scope (computed once in app.blade.php):
      $navRole, $canPayroll, $canInventory, $canInventoryReports, $canManufacturing, $canMaintenance, $canStorefront,
      $navAlertCount, $navPendingRestocks, $navPendingStorefrontOrders,
      $navInSales, $navInTax, $navInPayroll,
      $navInInventory, $navInManufacturing, $navInMaintenance, $navInStorefront, $navInReports, $navInSettings
--}}
@php
...
<nav class="mt-3 flex-1 flex flex-col gap-0.5 px-2 pb-4 overflow-y-auto">
    ...
    {{-- ── Dashboard ───────────────────────────────────────────────────────── --}}
    <a href="{{ route('dashboard') }}" data-tour="nav-dashboard"
    ...
    <div x-data="{ open: @js($navInSales) }">
        <button type="button" @click="open = !open" data-tour="nav-sales" class="{{ $grp }}">
        ...
        <div x-show="open" x-collapse class="mt-0.5 space-y-0.5">
            ...
            <a href="{{ route('vendors.index') }}"
               class="{{ $sub }} {{ request()->routeIs('vendors.*') ? $on : $off }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 21V8a2 2 0 00-2-2H7a2 2 0 00-2 2v13m14 0H5m14 0h2M5 21H3m8-14v1m0 4v1m0 4v1m-4-6v1m0 4v1m8-6v1m0 4v1"/></svg>
                Vendors
            </a>
        </div>
    </div>
    ...
    <div x-data="{ open: @js($navInTax) }">
        <button type="button" @click="open = !open" data-tour="nav-tax" class="{{ $grp }}">
            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    <div x-data="{ open: @js($navInInventory) }">
        ...
        <div x-show="open" x-collapse class="mt-0.5 space-y-0.5">
            ...
            {{-- Supplier Bills --}}
            <a href="{{ $canInventory ? route('inventory.bills.index') : route('billing').'?upgrade_feature=inventory' }}"
               class="{{ $sub }} {{ $canInventory ? (request()->routeIs('inventory.bills.*') ? $on : $off) : $dim }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/></svg>
                Supplier Bills
                @if($canInventory && ($navOutstandingBills ?? 0) > 0)
    ...
    {{-- ── Maintenance ───────────────────────────────────────────────────── --}}
    <div x-data="{ open: @js($navInMaintenance) }">
        <button type="button" @click="open = !open" class="{{ $grp }}">
            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
        ...
        <div x-show="open" x-collapse class="mt-0.5 space-y-0.5">
            <a href="{{ $canMaintenance ? route('maintenance.dashboard') : route('billing').'?upgrade_feature=maintenance' }}"
               class="{{ $sub }} {{ $canMaintenance ? (request()->routeIs('maintenance.dashboard') ? $on : $off) : $dim }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7" rx="1" stroke="currentColor" stroke-width="2"/><rect x="14" y="3" width="7" height="7" rx="1" stroke="currentColor" stroke-width="2"/><rect x="14" y="14" width="7" height="7" rx="1" stroke="currentColor" stroke-width="2"/><rect x="3" y="14" width="7" height="7" rx="1" stroke="currentColor" stroke-width="2"/></svg>
                Overview
                @unless($canMaintenance)<svg class="ml-auto w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>@endunless
            </a>
            <a href="{{ $canMaintenance ? route('maintenance.assets.index') : route('billing').'?upgrade_feature=maintenance' }}"
               class="{{ $sub }} {{ $canMaintenance ? (request()->routeIs('maintenance.assets.*') ? $on : $off) : $dim }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                Assets
                @unless($canMaintenance)<svg class="ml-auto w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>@endunless
            </a>
            <a href="{{ $canMaintenance ? route('maintenance.work-orders.index') : route('billing').'?upgrade_feature=maintenance' }}"
               class="{{ $sub }} {{ $canMaintenance ? (request()->routeIs('maintenance.work-orders.*') ? $on : $off) : $dim }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>
                Work Orders
                @unless($canMaintenance)<svg class="ml-auto w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>@endunless
            </a>
            <a href="{{ $canMaintenance ? route('maintenance.schedules.index') : route('billing').'?upgrade_feature=maintenance' }}"
               class="{{ $sub }} {{ $canMaintenance ? (request()->routeIs('maintenance.schedules.*') ? $on : $off) : $dim }}">
            ...
            <a href="{{ $canMaintenance ? route('maintenance.breakdowns.index') : route('billing').'?upgrade_feature=maintenance' }}"
               class="{{ $sub }} {{ $canMaintenance ? (request()->routeIs('maintenance.breakdowns.*') ? $on : $off) : $dim }}">
    ...
    {{-- ── Online Storefront ─────────────────────────────────────────────── --}}
    <div x-data="{ open: @js($navInStorefront) }">
        <button type="button" @click="open = !open" class="{{ $grp }}">
            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z"/>
            </svg>
            <span class="flex-1 text-left">Storefront</span>
            @unless($canStorefront)
                <svg class="w-3 h-3 text-green-400/70 flex-shrink-0 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
            @else
                {!! $pill($navPendingStorefrontOrders) !!}
            @endunless
            <svg class="{{ $chv }}" :class="open && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
            </svg>
        ...
        <div x-show="open" x-collapse class="mt-0.5 space-y-0.5">
            <a href="{{ $canStorefront ? route('storefront.orders.index') : route('billing').'?upgrade_feature=storefront' }}"
               class="{{ $sub }} {{ $canStorefront ? (request()->routeIs('storefront.orders.*') ? $on : $off) : $dim }}">
                ...
                Orders
                @if($canStorefront) {!! $pill($navPendingStorefrontOrders) !!} @else
                <svg class="ml-auto w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                @endif
            </a>
            <a href="{{ $canStorefront ? route('storefront.products.index') : route('billing').'?upgrade_feature=storefront' }}"
               class="{{ $sub }} {{ $canStorefront ? (request()->routeIs('storefront.products.*') ? $on : $off) : $dim }}">
                ...
                Products
                @unless($canStorefront)<svg class="ml-auto w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>@endunless
            </a>
            <a href="{{ $canStorefront ? route('storefront.services.index') : route('billing').'?upgrade_feature=storefront' }}"
               class="{{ $sub }} {{ $canStorefront ? (request()->routeIs('storefront.services.*') ? $on : $off) : $dim }}">
            ...
            <a href="{{ $canStorefront ? route('storefront.categories.index') : route('billing').'?upgrade_feature=storefront' }}"
               class="{{ $sub }} {{ $canStorefront ? (request()->routeIs('storefront.categories.*') ? $on : $off) : $dim }}">
            ...
            <a href="{{ $canStorefront ? route('storefront.settings') : route('billing').'?upgrade_feature=storefront' }}"
               class="{{ $sub }} {{ $canStorefront ? (request()->routeIs('storefront.settings*') ? $on : $off) : $dim }}">
    ...
    <div x-data="{ open: @js($navInReports) }">
        <button type="button" @click="open = !open" data-tour="nav-reports" class="{{ $grp }}">
            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    <a href="{{ route('help.index') }}" data-tour="nav-help"
       class="{{ $lnk }} {{ request()->routeIs('help.*') ? $on : $off }}">
        <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    {{-- ── Replay onboarding tour (all roles) ────────────────────────────────── --}}
    <button type="button" onclick="window.startAppTour && window.startAppTour(true)" class="{{ $lnk }} {{ $off }} w-full">
        <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div x-data="{ open: @js($navInSettings) }">
        <button type="button" @click="open = !open" data-tour="nav-settings" class="{{ $grp }}">
            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
        ...
        <div x-show="open" x-collapse class="mt-0.5 space-y-0.5">
            ...
            <a href="{{ route('settings.chart-of-accounts.index') }}"
               class="{{ $sub }} {{ request()->routeIs('settings.chart-of-accounts*') ? $on : $off }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Chart of Accounts
            </a>
            ...
            <a href="{{ route('referrals.index') }}"
               class="{{ $sub }} {{ request()->routeIs('referrals.*') ? $on : $off }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>
                Referrals &amp; Credits
            </a>
        </div>
    </div>
