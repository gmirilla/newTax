...
<div class="p-6 max-w-3xl mx-auto">
    ...
    <div class="mb-6">
        <a href="{{ route('maintenance.assets.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Back to Assets</a>
        <h1 class="text-2xl font-bold text-gray-900 mt-1">Add Asset</h1>
    </div>

    <form method="POST" action="{{ route('maintenance.assets.store') }}" class="space-y-6">
        @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="asset_name" value="{{ old('asset_name') }}" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent @error('asset_name') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="manufacturer" value="{{ old('manufacturer') }}" maxlength="100"
                ...
                <div>
                    ...
                    <input type="text" name="serial_number" value="{{ old('serial_number') }}" maxlength="100"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500">
                </div>
                ...
                <div>
                    ...
                    <select name="assigned_operator_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500">
                        ...
                        <option value="{{ $op->id }}" @selected(old('assigned_operator_id') == $op->id)>{{ $op->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        PM Interval (days)
                        <span class="text-gray-400 font-normal">(default for schedules)</span>
                    </label>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="notes" rows="3" maxlength="2000"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500">{{ old('notes') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('maintenance.assets.index') }}" class="px-6 py-2.5 border border-gray-300 text-sm text-gray-700 rounded-lg hover:bg-gray-50">
                Cancel
            </a>
        </div>
    </form>
