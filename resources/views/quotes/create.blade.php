...
@section('content')
<div x-data="quoteForm()" class="space-y-6" @keydown.escape="showNewCustomer = false; showPreview = false">

    <form x-ref="mainForm" method="POST" action="{{ route('quotes.store') }}" @submit="calculateTotals">
        @csrf
                            ...
                            <tr class="border-b">
                                ...
                                <td class="py-2 pr-2">
                                    <input type="number" :name="`items[${index}][quantity]`"
                                           x-model="item.quantity" @input="calculateItem(index)" min="1" step="1"
    ...
    <template x-teleport="body">
        <div x-show="showPreview" x-cloak
            ...
            <div class="flex items-center justify-between bg-gray-900 px-5 py-3 shrink-0">
                <span class="text-white text-sm font-medium">Proforma Invoice Preview</span>
...
function quoteForm() {
    return {
        ...
        async openPreview() {
            ...
            try {
                const res = await fetch('{{ route('quotes.preview') }}', {
                    method: 'POST',
