...
<h1 class="font-display text-2xl font-semibold text-gray-900 tracking-tight mb-7">Your Cart</h1>
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm divide-y divide-gray-100">
    ...
    <div class="flex items-center gap-4 p-4">
        <div class="w-16 h-16 rounded-xl bg-gray-50 flex-shrink-0 overflow-hidden flex items-center justify-center">
            @if($image)
        ...
        <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-500 mt-0.5">{{ rtrim(rtrim(number_format($line->quantity, 2), '0'), '.') }} × ₦{{ number_format($line->unit_price, 2) }}</p>
        </div>
...
<div class="mt-6 bg-white rounded-2xl border border-gray-100 shadow-sm p-5 max-w-sm ml-auto space-y-2 text-sm">
...
<div class="mt-6 flex justify-end">
    <a href="{{ route('storefront.checkout', $tenant->slug) }}"
       ...
       class="inline-flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-semibold shadow-sm hover:shadow-md hover:-translate-y-px transition-all">
        Proceed to Checkout
