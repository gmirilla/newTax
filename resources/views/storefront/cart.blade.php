    ...
    <div class="flex items-center gap-4 p-4">
        <div class="w-14 h-14 rounded-lg bg-gray-50 flex-shrink-0 overflow-hidden flex items-center justify-center">
            @if($line->product->images->first())
                <img src="{{ Storage::url($line->product->images->first()->image_path) }}" class="w-full h-full object-cover">
            @else
        ...
        <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-gray-900 truncate">{{ $line->product->item->name }}</p>
        ...
        <form method="POST" action="{{ route('storefront.cart.remove', $tenant->slug) }}">
            @csrf
            <input type="hidden" name="storefront_product_id" value="{{ $line->product->id }}">
