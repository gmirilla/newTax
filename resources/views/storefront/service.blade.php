...
<div class="grid md:grid-cols-2 gap-12">
    <div>
        ...
        <div x-data="{ active: 0 }">
            <div class="aspect-square bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
                ...
                <img x-show="active === {{ $i }}" x-transition.opacity src="{{ Storage::url($image->image_path) }}" alt="{{ $service->name }}" class="w-full h-full object-cover">
                @endforeach
            </div>
    ...
    <div>
        ...
        @endphp
        <span class="inline-block text-[10px] font-semibold uppercase tracking-wide px-2.5 py-1 rounded-full text-white mb-3" style="background-color: {{ $tenant->accentColor() }};">Service</span>
        <h1 class="font-display text-2xl sm:text-3xl font-semibold text-gray-900 tracking-tight">{{ $service->name }}</h1>
        ...
        <p class="mt-5 text-sm text-gray-500 leading-relaxed">{{ $service->description }}</p>
        @endif
