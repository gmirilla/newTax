    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="flex items-center gap-2" x-data="{ copied: false }">
            <input type="text" readonly value="{{ route('storefront.index', $tenant->slug) }}" onclick="this.select()"
                   x-ref="link" class="flex-1 rounded-md border-gray-300 bg-gray-50 shadow-sm text-sm font-mono text-gray-700">
        ...
        <form method="POST" action="{{ route('storefront.settings.update') }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="4" placeholder="Tell customers about your business..."
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm focus:ring-green-500 focus:border-green-500">{{ old('description', $storefront->description) }}</textarea>
                @error('description')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>

            <div class="flex justify-end pt-2">
                <button type="submit" class="px-6 py-2 bg-green-600 text-white text-sm font-medium rounded-md hover:bg-green-700">
                    Save Settings
                </button>
            </div>
        </form>
        ...
        <div class="flex items-start gap-6">
            <div class="flex-shrink-0">
                @if($storefront->banner_image)
                    <img src="{{ Storage::url($storefront->banner_image) }}" alt="Banner" class="h-20 w-40 object-cover border border-gray-200 rounded bg-gray-50">
                @else
                    <div class="h-20 w-40 border-2 border-dashed border-gray-300 rounded flex items-center justify-center text-gray-400 text-xs text-center bg-gray-50">No banner</div>
                @endif
            </div>
                ...
                <form method="POST" action="{{ route('storefront.settings.banner.upload') }}" enctype="multipart/form-data" class="flex items-end gap-3">
                    ...
                    <div class="flex-1">
                        <input type="file" name="banner" accept="image/*" required
                               class="block w-full text-sm text-gray-600 file:mr-3 file:py-1.5 file:px-4 file:rounded file:border-0 file:text-sm file:font-medium file:bg-green-50 file:text-green-700 hover:file:bg-green-100">
                        @error('banner')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <button type="submit" class="px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-md hover:bg-green-700">Upload</button>
                </form>
                @if($storefront->banner_image)
